...
'''
# 1. 추출한 결과를 위 리스트에 저장한다
callLists = soup.select('.call-list .tabcont')

for callList in callLists:
    # 구청 이름
    구청이름.append(callList.select_one('strong').text)
    구청주소.append(callList.select('ul li')[0].text)
    구청전화번호.append(callList.select('ul li')[1].text)
    구청홈페이지.append(callList.select_one('ul li a').text)
